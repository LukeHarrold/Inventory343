...
class apiTest(unittest.TestCase):
    ...
    def test_send_part_information(self):
        ...
        for part in data: 
            self.assertEqual(part[0]['fields']['partTypeId'], 2)
    ...
    def test_return_specific_model(self):
        ...
        data = json.loads(resp.text.decode('utf-8'))
        self.assertEqual(resp.status_code,200)
    ...
    def test_mark_as_bogo(self):
        resp = self.app.get('/inventory/phone/mark_bogo/{}'.format(1))
        data = json.loads(resp.data.decode('utf-8'))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(data['bogo'], 1)

    def test_get_phone_by_id(self):
        resp = self.app.get('/inventory/phones/{}'.format(1))
        data = json.loads(resp.data.decode('utf-8'))
        self.assertEqual(resp.status_code, 200)
